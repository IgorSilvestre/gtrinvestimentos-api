...
export async function getBrazilStates(): Promise<Record<string, any> | AppError>{
  const cachekey = `getBrazilStates`
  const cachedData = CACHE.get(cachekey)
  if (cachedData) return cachedData

  try {
    const response = await axios.get(
      externalAPIEndpoints.locations.url.getBrazilStates,
