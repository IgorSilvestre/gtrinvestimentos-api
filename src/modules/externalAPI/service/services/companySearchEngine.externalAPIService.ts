...
function parseCompany (company: ICompanySearchEngineData) {
  return {
    ...company,
    full_address: company?.full_address?.replace(new RegExp("^" + company.name + " - "), "")
  }
}
...
export async function companySearchEngine(query: string) {
  ...
  try {
    const response = await axios.request(
      externalAPIConfigs.companySearchEngine.defaultOptions as AxiosRequestConfig
    )
    const companies = response.data
    return companies.data.map((company: ICompanySearchEngineData) => parseCompany(company))
