...
export async function search (searchParams: ISearchParams): Promise<ZPersonModel[] | AppError> {
  try {
    const people: ZPersonModel[] | null = await PersonRepository.search(searchParams)

    if (people === null) return new AppError(
